...
### 3.1 RouteSelector 职责

`RouteSelector` 是系统的"大脑"，负责根据文档特征选择合适的提取器。
...
```python
def select_extractor(document) -> Tuple[Extractor, str, float, bool]:
    """
    Returns:
        (extractor, product_type, confidence, is_hybrid)
    """
...
**设计亮点**: 直接返回提取器实例，调用方无需根据 `mode` 字段二次判断。
...
```
        ...
        ▼                    ▼                    ▼
┌───────────────┐  ┌──────────────────┐  ┌──────────────┐
│   Normalizer  │  │  RouteSelector   │  │   Validator  │
└───────────────┘  └────────┬─────────┘  └──────────────┘
                             │
                ┌────────────┴────────────┐
                │                         │
...
| `classifier.py` | `models`, `product_types` | `RouteSelector` | ✅ 低 |
| `route_selector.py` | `models`, `classifier`, 提取器 | `DocumentExtractor` | ⚠️ 中 (持有提取器) |
| `fast_extractor.py` | `models` | `RouteSelector` | ✅ 低 |
| `dynamic_extractor.py` | `models`, `prompt_builder`, `product_types` | `RouteSelector` | ✅ 低 |
| `validator.py` | `models`, `route_selector` | `DocumentExtractor` | ⚠️ 中 (依赖 RouteSelector 常量) |
| `document_extractor.py` | 所有模块 | 外部调用 | ✅ 低 (编排层) |

**唯一的中度耦合**:
- `RouteSelector` 需要持有提取器实例（方案 A 的代价）
- `validator` 依赖 `RouteSelector.REQUIRED_FIELDS`（可优化）
...
| 问题 | 严重性 | 说明 |
|------|--------|------|
| `validator` 依赖 `RouteSelector` | P3 | 循环依赖风险，`REQUIRED_FIELDS` 应该独立配置 |
| `RouteSelector` 持有提取器实例 | P3 | 增加耦合，但换取了接口简洁度 |
...
1. **单一职责**: 每个类职责明确（Normalizer 只负责规范化，RouteSelector 只负责路由）
2. **依赖注入**: 提取器通过构造函数注入 LLM 客户端
3. **防御性编程**: Fast 提取失败有回退机制，验证器有多层检查
4. **日志完备**: 关键决策点有日志记录（DEBUG 模式下有决策理由）
...
1. **接口简洁**: `RouteSelector.select_extractor()` 直接返回提取器实例
2. **模型清晰**: `DocumentProfile` 准确表达了文档画像的职责
3. **无冗余**: 删除了 `ExtractionStrategy`、`DecisionDiagnostics`、`StructureInfo` 等中间类
4. **数据流清晰**: 每个阶段的输入输出都有明确的类型定义
...
3. **调整路由规则**: 修改 `RouteSelector._use_dynamic()`
