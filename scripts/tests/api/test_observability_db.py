...
class TestSessionSearch:
    def test_search_sessions_no_filter(self, _patch_database, make_session):
        import api.database as db
        make_session("sess_aaa", "健康保险等待期")
        make_session("sess_bbb", "免责条款查询")
        make_session("sess_ccc", "等待期相关问题")
        rows = db.search_sessions(search="", page=1, size=10)
        assert rows[1] == 3  # total count

    def test_search_sessions_by_title(self, _patch_database, make_session):
        import api.database as db
        make_session("sess_aaa", "健康保险等待期")
        make_session("sess_bbb", "免责条款查询")
        make_session("sess_ccc", "等待期相关问题")
        rows = db.search_sessions(search="等待期", page=1, size=10)
        assert rows[1] == 2
        titles = [r["title"] for r in rows[0]]
        assert "健康保险等待期" in titles
        assert "等待期相关问题" in titles

    def test_search_sessions_pagination(self, _patch_database, make_session):
        import api.database as db
        for i in range(5):
            make_session(f"sess_{i}", f"对话 {i}")
        rows = db.search_sessions(search="", page=1, size=2)
        assert len(rows[0]) == 2
        assert rows[1] == 5


class TestBatchDeleteSessions:
    def test_batch_delete(self, _patch_database, make_session, make_message):
        import api.database as db
        make_session("sess_del1", "删除1")
        make_message("sess_del1", "user", "问题1")
        make_message("sess_del1", "assistant", "回答1")
        make_session("sess_del2", "删除2")
        make_message("sess_del2", "user", "问题2")
        make_session("sess_keep", "保留")
        deleted = db.batch_delete_sessions(["sess_del1", "sess_del2"])
        assert deleted == 2
        remaining = db.get_sessions()
        assert len(remaining) == 1
        assert remaining[0]["id"] == "sess_keep"


class TestTraceSearch:
    def _create_trace(self, trace_id="t1", message_id=1, session_id="sess_a", status="ok"):
        import api.database as db
        span = {
        ...
        db.save_trace(trace_id, message_id, session_id,
    ...
    def test_search_traces_by_session_id(self, _patch_database):
        import api.database as db
        self._create_trace("t1", session_id="sess_x")
        self._create_trace("t2", session_id="sess_y")
        rows, total = db.search_traces(session_id="sess_x", page=1, size=10)
        assert total == 1
...
class TestGetTraceByTraceId:
    def test_get_existing_trace(self, _patch_database):
        import api.database as db
        db.save_trace("abc123", 1, "sess_test")
        db.save_spans([{
...
class TestCleanupTraces:
    def test_count_and_cleanup(self, _patch_database):
        ...
        db.save_trace("t3", 3, "sess_c", status="error")
