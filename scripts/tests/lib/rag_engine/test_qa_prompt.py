...
class TestPromptSeparation:
    ...
    def test_user_prompt_contains_context_and_question(self, config):
        results = [{'law_name': '测试', 'article_number': '第一条', 'content': '内容'}]
        prompt, count = RAGEngine._build_qa_prompt(config, "测试", results)
    ...
    def test_user_prompt_has_trailing_reminder(self, config):
        results = [{'law_name': '测试', 'article_number': '第一条', 'content': '内容'}]
        prompt, _ = RAGEngine._build_qa_prompt(config, "测试", results)
