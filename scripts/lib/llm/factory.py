...
class LLMClientFactory:
    ...
    @staticmethod
    def _create_client(provider: str, scene: dict) -> BaseLLMClient:
        """根据 provider 和场景配置创建客户端（内部方法）"""
        from lib.config import get_config
        cfg = get_config()

        if provider == 'zhipu':
            model = scene.get('model') or cfg._zhipu.chat_model
            timeout = scene.get('timeout') or cfg._zhipu.timeout
            return ZhipuClient(
            ...
            model = scene.get('model') or cfg._ollama.chat_model
            timeout = scene.get('timeout') or cfg._ollama.timeout
            return OllamaClient(
    ...
    @staticmethod
    def _get_chat_scene(scene_name: str) -> BaseLLMClient:
        """获取聊天场景客户端（内部方法）"""
        from lib.config import get_config
        cfg = get_config()
        scene = cfg._llm.scenes.get(scene_name, {})
        provider = cfg._llm.chat.get('provider', 'zhipu')
        return LLMClientFactory._create_client(provider, scene)

    @staticmethod
    def get_qa_llm() -> BaseLLMClient:
        """获取问答场景 LLM"""
        return LLMClientFactory._get_chat_scene('qa')

    @staticmethod
    def get_audit_llm() -> BaseLLMClient:
        """获取审计场景 LLM"""
        return LLMClientFactory._get_chat_scene('audit')

    @staticmethod
    def get_eval_llm() -> BaseLLMClient:
        """获取评估场景 LLM（RAGAS 等评估框架使用）"""
        return LLMClientFactory._get_chat_scene('eval')

    @staticmethod
    def get_doc_preprocess_llm() -> BaseLLMClient:
        """获取文档预处理场景 LLM"""
        return LLMClientFactory._get_chat_scene('doc_preprocess')
