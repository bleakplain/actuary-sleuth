...
class MinimaxClient(BaseLLMClient):
    ...
    def _register_cleanup(self):
        ...
        if cleanup not in MinimaxClient._shutdown_hooks:
            MinimaxClient._shutdown_hooks.append(cleanup)
            atexit.register(cleanup)

    @staticmethod
    def _strip_thinking(content: str) -> str:
        """去除 MiniMax thinking 标签，返回纯内容。"""
        import re
    ...
    @_track_timing("minimax")
    @_with_circuit_breaker("minimax")
    @_retry_with_backoff(
