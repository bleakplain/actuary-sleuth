...
from api.database import save_compliance_report, get_parsed_document
...
def _run_compliance_check(engine, prompt: str, search_results: list = None) -> Dict:
...
def _detect_missing_clauses(parsed_doc: Dict, check_result: Dict) -> List[Dict]:
    """检测文档中存在但未被检查覆盖的条款"""
    checked_numbers = {
    ...
    for clause in parsed_doc.get("clauses", []):
        ...
        if number and number not in checked_numbers:
            missing_items.append({
                "clause_number": number,
                "param": f"条款 {number} {clause.get('title', '')}",
                "value": "-",
