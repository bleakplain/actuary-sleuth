...
```
[可选] /gen-specify  — 需求 → spec.md（主仓库创建 specs）
/gen-research        — 代码/需求分析 → research.md（主仓库）
/gen-plan            — 技术方案 → plan.md（主仓库）
/exec-plan           — 提交推送 specs → 创建 worktree → 任务分解 + 实现
/fix-plan            — 审查 + 批注处理（主仓库）
```

### Specs 管理策略

- **specs 在主仓库管理** — gen-specify、gen-research、gen-plan、fix-plan 都在主仓库操作
- **exec-plan 负责提交和 worktree** — 执行前自动提交 specs 到 master，再创建 worktree
- worktree 目录：`.Codex/worktrees/<feature-name>/`
- 分支编号：扫描本地+远程分支取 max+1，格式 `NNN-feature-name`
- 分支名 = `.Codex/specs/` 子目录名

### 产物规范

所有 SDD 产物统一在主仓库 `.Codex/specs/<feature-name>/` 下：

```
.Codex/specs/<feature-name>/
├── spec.md          # gen-specify 输出（可选）
├── research.md      # gen-research 输出
├── plan.md          # gen-plan 输出
└── tasks.md         # exec-plan 生成（worktree 中）
```

### 模式检测

从 `.Codex/specs/` 目录检测 feature-name，检查对应的 `spec.md` 是否存在：
- 存在 → SDD 模式（需求驱动）
