...
**Knowledge base**: v5，25 documents / 172 chunks
**Primary constraints**:
...
| 8 | 已冻结23份真实产品的文件身份和候选输入；产品标签、法规适用性、条款路由和审核结论仍待精算标注签收 | 输入已冻结，业务标注 pending |
...
1. `compliance-audit-v1` 的产品标签、法规适用性、条款路由和审核结论仍待精算标注；
...
- 只使用2026 Excel 安全重建并切换 v5：25个文档、172个 LanceDB chunks、172个 BM25 文档，双索引身份一致；
...
- 生成 `精算审核真值验收表_阶段一_v2.xlsx`，保留旧人工反馈并把受系统修正影响的记录标为待复核；
