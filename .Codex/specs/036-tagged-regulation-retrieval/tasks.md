...
- [ ] T100 [US4] 修改 `audit_pipeline.py` 审核包构建，保留路由关系但不因 `not_relevant` 将条款标为未提交
- [ ] T101 [US4-US5] 确保确定性事实和超长分段覆盖全部产品条款，分段并集不丢 `clause_id`
- [ ] T102 [US6] API 将路由分类与实际 `submitted` 分开返回，完整条款基线下所有块均为已提交
- [ ] T103 [US3-US6] 补充明确不相关块仍提交、全文事实提取、超长分段全覆盖和 API 证据轨迹回归测试
- [ ] T104 [US4-US6] 运行相关测试、全库基线、类型检查和差异审查，记录环境既有失败
